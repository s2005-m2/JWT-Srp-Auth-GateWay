...
pub async fn rate_limit_middleware(
    ...
    let key = match extract_client_ip(&request) {
        Some(ip) => ip,
        None => {
            tracing::warn!("Request rejected: unable to determine client IP (ConnectInfo not configured)");
            return (
    ...
    for trusted in TRUSTED_PROXIES {
        if trusted.contains('/') {
            if let Some((network, prefix_len)) = trusted.split_once('/') {
                if let (Ok(net_ip), Ok(prefix)) = (network.parse::<IpAddr>(), prefix_len.parse::<u8>()) {
